...
def computeDelta(wt, X, Xi):
    ...
    # A function to get similarity which will replace the euclidean distance in equation 6
    def getSimilarity(a,b):
    	mean_a = np.mean(a)
    	mean_b = np.mean(b)
    	std_a = np.std(a)
    	std_b = np.std(b)
    	s = sum((a-mean_a) * (b-mean_b))*(1.0)/(len(a) * std_a * std_b)
    	return s

    numerator = 0
    denominator = 0

    # Computation for equation 6 of the paper.
    for i in range(len(Xi)):
        y_i = Xi.iloc[i][-1]
        x_i = Xi.iloc[i][:-1]
        similarity = getSimilarity(X[:-1],x_i)
        numerator +=  y_i * math.exp(similarity * wt)
        denominator += math.exp(similarity * wt)

    output = numerator*(1.0) / denominator
    return output
